...
public class TprReportServiceImpl implements TprReportService {
    ...
    public Map day_getBriefinfobyprojectname(String projectname) {
        String projectid=tprReportMapper.getidbyname(projectname);
            ...
            for (int j=0;j<namelist.size();j++){
                ...
                try{
                    if (runedcaselist.get(j).get("moudlename").equals(namelist.get(i))){
                        ...
                        String num1 = df.format(num)+"%";//返回的是String类型
                        test.put("runedpercent",num1);
                    }
    ...
    public Map week_getBriefinfobyprojectname(Map info) {
        String projectid=tprReportMapper.getidbyname((String) info.get("projectname"));
        ...
        //获取各模块本周已执行案例数
        List<Map> runnumberlist=tprReportMapper.getweekrunnumber(temp);
    ...
    public Workbook exportMoreSheetByTemplateday(String projectId) {
        ...
        //获取本日解决缺陷数（各模块数）
        List<Map> bugsolvednumberlist=tprReportMapper.getweekbugsolvednumber(temp);
        ...
        for (int i=0;i<namelist.size();i++){
            ...
            for (int j=0;j<namelist.size();j++){
                //如果模块名字相同，则放入数据
                try{
                    if (bugsolvednumberlist.get(j).get("moudlename").equals(namelist.get(i))){
                        test.put("solvednumber", bugsolvednumberlist.get(j).get("number"));
                    }
                } catch (IndexOutOfBoundsException e) {

                }finally {
                    if (!test.containsKey("solvednumber"))
                    {
                        //模块名字不同装入0
                        test.put("solvednumber",0);
                    }
                }
